        ...
        >
          ...
          <div class="pro-num">
            ...
            <el-input-number
              ...
              :value="Number(item.cartNum)"
              @change="handleChange($event, index, item.productId,item.cartId)"
              :min="1"
              :max="10"
            ></el-input-number>
...
export default {
  ...
  methods: {
    ...
    // 修改商品数量的时候调用该函数
    handleChange(currentValue, key, productID,cartId) {
      ...
      // 向后端发起更新购物车的数据库信息请求
      this.$axios.put("/api/shopping-cart/", {
        userId: this.$store.getters.getUser.userId,
        productId: productID,
        cartNum: currentValue,
        cartId:cartId
        })
        .then((res) => {
    ...
    checkChange(val, key) {
      // 更新vuex中购物车商品是否勾选的状态
      this.updateShoppingCart({ key: key, prop: "checkbox", val: val });
